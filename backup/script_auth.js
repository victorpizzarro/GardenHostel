 ...
 * (Versão 6.0 - Com Tradução e Validação)
 */
...
// 1. Pega o idioma salvo no 'index.html'
...
function aplicarTraducoes() {
    ...
    // Adiciona a classe no body para evitar a "piscada" (FOUC)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    aplicarTraducoes(); // Aplica a tradução salva
    ...
    // Adiciona listeners nas bandeiras
    ...
    btnLogin.addEventListener('click', () => {
        ...
        if (!email || !senha) {
            exibirErro(loginErrorMessage, 'Email e senha são obrigatórios.');
            return;
        }
        fetch('api/usuario/login.php', {
            ...
            if (data.status === 'sucesso') {
                window.location.href = (data.tipo_usuario === 'CLIENTE') ? 'painel_cliente.html' : 'painel_admin.html';
            } else {
    ...
    regDocNum.addEventListener('input', (e) => {
        ...
        let valorFormatado = '';
        if (valor.length > 9) { 
            valorFormatado = valor.replace(/(\d{3})(\d{3})(\d{3})(\d{1,2})/, '$1.$2.$3-$4');
    ...
    regCelular.addEventListener('input', (e) => {
        ...
        if (valor.length === 11) { 
            ...
            valorFormatado = valor.replace(/(\d{2})(\d{4})(\d{1,4})/, '($1) $2-$3');
        } else if (valor.length > 2) { 
            valorFormatado = valor.replace(/(\d{2})(\d{1,4})/, '($1) $2');
        } else if (valor.length > 0) { 
            valorFormatado = valor.replace(/(\d{1,2})/, '($1');
        }
        e.target.value = valorFormatado; 
    });
    ...
    regNome.addEventListener('blur', () => {
        const nomeCompleto = regNome.value.trim().split(' ');
        if (!regNome.value.trim()) {
            mostrarErro('reg-nome', 'Preencha este campo.');
        } else if (nomeCompleto.length < 2 || nomeCompleto[0].length < 3) {
            mostrarErro('reg-nome', 'Nome e sobrenome obrigatórios (mín. 3 letras no nome).');
        } else {
            limparErro('reg-nome');
        }
    });
    ...
    regEmail.addEventListener('blur', () => {
        ...
        if (!regEmail.value.trim()) {
            mostrarErro('reg-email', 'Preencha este campo.');
        } else if (!emailRegex.test(regEmail.value)) {
            mostrarErro('reg-email', 'Formato de email inválido.');
        } else {
            limparErro('reg-email');
        }
    });

    regDocNum.addEventListener('blur', () => {
        const valor = regDocNum.value;
        if (!valor.trim()) {
            mostrarErro('reg-doc-num', 'Preencha este campo.');
        } else if (regDocTipo.value === 'CPF' && valor.length !== 14) { 
            mostrarErro('reg-doc-num', 'CPF inválido (deve ter 11 dígitos).');
        } else {
            limparErro('reg-doc-num');
        }
    });
    
    regNascimento.addEventListener('blur', () => {
        if (!regNascimento.value) {
            mostrarErro('reg-nascimento', 'Preencha este campo.');
        } else {
            const ano = new Date(regNascimento.value).getFullYear();
            if (isNaN(ano) || ano < 1900 || ano > new Date().getFullYear()) {
                mostrarErro('reg-nascimento', 'Data inválida (ano deve ter 4 dígitos e ser válido).');
            } else {
    ...
    regCelular.addEventListener('blur', () => {
        const valor = regCelular.value;
        if (!valor.trim()) {
            mostrarErro('reg-celular', 'Preencha este campo.');
        } else if (valor.length < 14) { 
            mostrarErro('reg-celular', 'Celular incompleto.');
        } else {
    ...
    dataFieldWrapper.addEventListener('click', (e) => {
        try {
            regNascimento.showPicker();
        } catch (error) {
            regNascimento.focus();
        }
    });

    regNascimento.addEventListener('keydown', (e) => {
        if (e.key !== 'Tab') {
            e.preventDefault();
        }
    });
    ...
    regSenha.addEventListener('input', () => limparErroGeral(registerErrorMessage));
    ...
    btnRegister.addEventListener('click', () => {
        ...
        // Roda TODAS as validações de novo (força o "blur")
        ...
        // Se tudo OK, prepara os dados para a API
        const dadosCadastro = {
        ...
        .then(data => {
            if (data.status === 'sucesso') {
                ...
                exibirErro(registerErrorMessage, data.mensagem);
            }
        })
        .catch(error => {
            console.error('Erro no fetch de cadastro:', error);
            exibirErro(registerErrorMessage, 'Erro de conexão com a API.');
        });
    });
