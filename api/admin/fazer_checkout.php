...
$sql_checkout = "UPDATE Reservas SET status_reserva = 'FINALIZADA' WHERE id = ? AND status_reserva = 'CHECKIN'";
$stmt = $conexao->prepare($sql_checkout);
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        ...
        http_response_code(400);
        echo json_encode(['status' => 'erro', 'mensagem' => 'Check-out falhou. Reserva não encontrada ou não estava "CHECKIN".']);
    }
} else {
    http_response_code(500);
    echo json_encode(['status' => 'erro', 'mensagem' => 'Erro ao processar check-out: ' . $stmt->error]);
}
